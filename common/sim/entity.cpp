...
void AnimateEntity(Entity* pent, int entindex)
{
	if(IsActor(pent))
	{
		if(pent->forward)
			PlayAnimation(pent->frame[0], 0, 29, true, 1.2f);
		else
			PlayAnimation(pent->frame[0], 0, 0, false, 1.2f);
	}
}

void UpdateEntities()
{
	for(int entindex = 0; entindex < ENTITIES; entindex++)
	{
		Entity* pent = g_entity[entindex];
		...
		MoveEntity(pent, entindex);
		AnimateEntity(pent, entindex);
	}
}