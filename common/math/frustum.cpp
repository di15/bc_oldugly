...
void Frustum::construct(const Plane left, const Plane right, const Plane top, const Plane bottom, const Plane front, const Plane back)
{
	...
	m_Frustum[TOP][A] = right.m_normal.x;
	m_Frustum[TOP][B] = right.m_normal.y;
	m_Frustum[TOP][C] = right.m_normal.z;
	m_Frustum[TOP][D] = right.m_d;
...
bool Frustum::BoxInFrustum2( float x, float y, float z, float x2, float y2, float z2)
{
	// Go through all of the corners of the box and check then again each plane
	// in the frustum.  If all of them are behind one of the planes, then it most
	// like is not in the frustum.
